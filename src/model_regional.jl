...
function residuals(rb::RegionalBaseline)
    ...
    Y = similar(rb.Y)
    _reg_output!(Y, rb.π_use, rb.π_fin, M, F, rb.τ, rb.ζ_trade)
...
# ── shared kernels ────────────────────────────────────────────────────────────────────────
# Defined locally so this branch stands alone; the Antràs–Chor branch has near-identical
# helpers, and the two should be reconciled if both are merged.

"""
    _reg_output!(Y, π_use, π_fin, M, F, τ, ζ)

`Y[o,j] = Σ_d ( π_fin[o,d,j]·F[d,j] + Σ_k π_use[o,d,j,k]·M[d,j,k] ) / (τ[o,d,j]·ζ[o,d,j])`,
where `F` is final demand and `M[d,j,k]` is `d`'s demand for sector-`j` goods as an input to its
sector `k`.
"""
function _reg_output!(Y, π_use, π_fin, M, F, τ, ζ)
    N, J = size(F)
    fill!(Y, 0.0)
...
function calibrate_regional(; regions, sectors, π_use, π_fin, βF, γL, γ, θ, F, L,
    ...
    _normalise_over_origins4!(pu); _normalise_trade_shares!(pf; verbose = 0)
    ...
    for it in 1:max_iterations
        ...
        _reg_output!(Ynew, pu, pf, M, F, τ, ζ_trade)
...
function _normalise_over_origins4!(pu)
    N, _, J, _ = size(pu)
...
function update_equilibrium(m::FelbermayrEtAl2025, rb::RegionalBaseline, sc::Scenario,
    ...
    π̄ = _reg_aggregate_shares(π_use′, π_fin′, ws.M′, ws.F′, X′)
...
function _reg_aggregate_shares(π_use, π_fin, M, F, X)
    N, _, J, _ = size(π_use)
    π̄ = zeros(N, N, J)
    @inbounds for j in 1:J, d in 1:N
        x = X[d, j]
